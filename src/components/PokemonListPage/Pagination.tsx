export function Pagination({next, previous, onPageChange, limit, offset, loading}: {
  next: string | null;
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 pb-12 flex justify-center gap-2">
      <button className={baseClass + (previous ? activeClass : inactiveClass)}
              disabled={!previous || loading}
              onClick={() => onPageChange(offset - limit)}
      >
        Previous
      </button>
      <button className={baseClass + (next ? activeClass : inactiveClass)}
